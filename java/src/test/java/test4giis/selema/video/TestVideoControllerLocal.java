...
import giis.selema.services.browser.Docker;
...
public class TestVideoControllerLocal {
	...
	protected void preloadRecorder(boolean stopAfterRun) {
		...
		for (int i = 1; i <= 5; i++) {
			if (CommandLine.fileExists(recordedVideo))
				...
				fail("Video file generated by the mock container after preload does not exist");
			log.warn("Try " + i + " Recorded video does not exist. Retry");
			JavaCs.sleep(1000);
		}
	...
	protected void doTestLifeCycle() {
		...
		assertEquals("exited", Docker.getContainerStatus(MOCK_CONTAINER));
	}
