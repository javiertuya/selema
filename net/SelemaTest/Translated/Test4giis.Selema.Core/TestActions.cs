	...
	[LifecycleNunit3] public class TestActions
	{
		...
		*/
		...
		public static void RunAlerts(IWebDriver driver, bool doAlert, bool doConfirm, bool doPrompt)
		{
			...
			if (doAlert)
			{
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("result", driver.FindElement(By.Id("spanAlert")).Text);
				...
				//click ok
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("clicked", driver.FindElement(By.Id("spanAlert")).Text);
			}
			//confirmaciones (si/no)
			if (doConfirm)
			{
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("result", driver.FindElement(By.Id("spanConfirm")).Text);
				...
				//confirmar
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("true", driver.FindElement(By.Id("spanConfirm")).Text);
				SeleniumActions.ClickActions(driver, driver.FindElement(By.Id("btnConfirm")));
				SeleniumActions.ManageAlert(driver, false, true, null, waitForClosedAlert, 0);
				//cancelar
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("false", driver.FindElement(By.Id("spanConfirm")).Text);
			}
			//prompts para introducir datos
			if (doPrompt)
			{
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("result", driver.FindElement(By.Id("spanPrompt")).Text);
				SeleniumActions.ClickActions(driver, driver.FindElement(By.Id("btnPrompt")));
				SeleniumActions.ManageAlert(driver, true, false, null, waitForClosedAlert, 0);
				//confirma sin incluir texto (por poner null)
				NUnit.Framework.Legacy.ClassicAssert.AreEqual(string.Empty, driver.FindElement(By.Id("spanPrompt")).Text);
				SeleniumActions.ClickActions(driver, driver.FindElement(By.Id("btnPrompt")));
				SeleniumActions.ManageAlert(driver, true, false, "text", waitForClosedAlert, 0);
				//confirma incluyendo texto
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("text", driver.FindElement(By.Id("spanPrompt")).Text);
				SeleniumActions.ClickActions(driver, driver.FindElement(By.Id("btnPrompt")));
				SeleniumActions.ManageAlert(driver, false, true, "text2", waitForClosedAlert, 0);
				//cancela (incluso habiendo puesto texto)
				NUnit.Framework.Legacy.ClassicAssert.AreEqual("NULL", driver.FindElement(By.Id("spanPrompt")).Text);
			}
		}
	}
}
